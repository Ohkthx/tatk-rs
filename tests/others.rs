...
fn create_variance_sample() {
    let variance = Variance::new(12, DATA, true).unwrap();
...
fn next_variance_sample() {
    let mut variance = Variance::new(12, DATA, true).unwrap();
    assert_eq!(variance.next(107.0), 0.9084750000000003)
}
...
fn create_variance_population() {
    let variance = Variance::new(12, DATA, false).unwrap();
...
fn next_variance_population() {
    let mut variance = Variance::new(12, DATA, false).unwrap();
    assert_eq!(variance.next(107.0), 0.8327687500000002)
}
...
fn create_stdev_sample() {
    let stdev = STDEV::new(12, DATA, true).unwrap();
...
fn next_stdev_sample() {
    let mut stdev = STDEV::new(12, DATA, true).unwrap();
    assert_eq!(stdev.next(107.0), 0.9531395490692851)
}
...
fn create_stdev_population() {
    let stdev = STDEV::new(12, DATA, false).unwrap();
...
fn next_stdev_population() {
    let mut stdev = STDEV::new(12, DATA, false).unwrap();
    assert_eq!(stdev.next(107.0), 0.912561641753586)
}
